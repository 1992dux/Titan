...
public class MongoDBConnection implements DBConnection{
	...
	public Set<Item> getFavoriteItems(String userId) {
		...
		Set<String> favoriteItemsId = getFavoriteItemIds(userId);
		for (String itemId : favoriteItemsId) {
	...
	public List<Item> searchItems(String userId, double lat, double lon, String term) {
		...
		for (Item item : items) {
			saveItem(item);				// save item in local MongoDB
		}
		return items;
	}
	...
	public void saveItem(Item item) {
		...
		db.getCollection("items").updateOne(new Document().append("item_id", item.getItemId()), 
				new Document("$set", new Document().append("item_id", item.getItemId())
															...
															.append("categories", item.getCategories())), options);
	}
